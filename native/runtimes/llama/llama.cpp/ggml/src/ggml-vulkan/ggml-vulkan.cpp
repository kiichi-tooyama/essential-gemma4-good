...
static uint64_t vk_log_handle(vk::Buffer buffer) {
...
void vk_memory_logger::log_allocation(vk_buffer_ref buf_ref, size_t size) {
    ...
    VK_LOG_MEMORY(buf->device->name << ": +" << format_size(size) << " " << type << " at " << vk_log_handle(buf->buffer) << ". Total device: " << format_size(total_device) << ", total host: " << format_size(total_host));
}
...
void vk_memory_logger::log_deallocation(vk_buffer_ref buf_ref) {
    ...
    if (it != allocations.end()) {
        VK_LOG_MEMORY(buf->device->name << ": -" << format_size(it->second) << " " << type << " at " << vk_log_handle(buf->buffer) << ". Total device: " << format_size(total_device) << ", total host: " << format_size(total_host));
        allocations.erase(it);
    } else {
        VK_LOG_MEMORY("ERROR " << buf->device->name << ": Attempted to deallocate unknown " << type << " memory at " << vk_log_handle(buf->buffer));
    }
}
